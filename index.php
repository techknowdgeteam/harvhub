<?php
    ...
    try {
        ...
        if ($config) {
            // Process brokers (e.g., "insiders: exness,insiders: bybit,insiders: deriv")
            $raw_brokers = explode(',', $config['brokers'] ?? '');
            
            // We want unique broker names used for 'insiders' as per context
            ...
            // Process broker links (e.g., "insiders: deriv.com, insiders: bybit.com")
            ...
            foreach ($raw_links as $entry) {
                if (preg_match('/(insiders):\s*(.+)/i', trim($entry), $matches)) {
                    ...
                    if (count($broker_link_parts) == 2) {
                        ...
                        // The user's input "insiders: deriv.com" only suggests deriv.com is the value, not key:value pair.
                        // I'll adjust the regex to better reflect a single key:value structure for the links, which is standard for config.
                        
                        // Re-evaluate based on the single example 'brokers_link' provided: "insiders: deriv.com"
                        // Assuming the broker name is 'Deriv' and the link is 'deriv.com'
                        // Since the regex captured 'deriv.com' as $matches[2] from "insiders: deriv.com"
        ...
        // Ensure that $allowed_brokers array contains unique broker names derived from 'insiders' entries
    ...
    if ($logged_in_email !== '') {
        ...
        $stmt = $pdo->prepare("SELECT application_status, broker, server, login FROM insiders WHERE email = ? LIMIT 1");
        ...
        if ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            // If the user's email is logged in but no row exists in the insiders table, create a row.
            try {
...
<body class="<?php echo ($logged_in_email !== '') ? 'logged-in' : ''; ?>">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Logic for 'Continue with your Email' or 'Join insiders Now'
            if (joinBtn) {
