...
<div class="revenue-container" id="revenue-container">
    ...
    <div id="tab-revenue-history" class="tab-content">
        ...
        <div class="search-bar search-bar-clickable" onclick="Revenue.showRevenueHistoryUsersModal()">
            <span class="search-icon">👤</span>
            <span class="search-placeholder" id="revenue-history-search-placeholder">Search users by name, email, or ID...</span>
        </div>

        <!-- Revenue History Summary Cards -->
        <div class="summary-cubes" id="revenue-summary-cubes">
            <div class="summary-cube">
                <div class="cube-value" id="rev-total-investment">$0.00</div>
                <div class="cube-label">Total Investment</div>
            </div>
            <div class="summary-cube">
                <div class="cube-value" id="rev-total-pnl">$0.00</div>
                <div class="cube-label">Total P&L</div>
            </div>
            <div class="summary-cube">
                <div class="cube-value" id="rev-total-server-share">$0.00</div>
                <div class="cube-label">Total Server Share</div>
            </div>
            <div class="summary-cube">
                <div class="cube-value" id="rev-total-user-share">$0.00</div>
                <div class="cube-label">Total Investors Share</div>
            </div>
            <div class="summary-cube">
                <div class="cube-value" id="rev-total-payments-made">$0.00</div>
                <div class="cube-label">Payments Made</div>
            </div>
            <div class="summary-cube">
                <div class="cube-value" id="rev-total-payments-confirmed">$0.00</div>
                <div class="cube-label">Payments Confirmed</div>
            </div>
            <div class="summary-cube">
                <div class="cube-value" id="rev-total-cancelled">$0.00</div>
                <div class="cube-label">Cancelled Contracts</div>
            </div>
            <div class="summary-cube">
                <div class="cube-value" id="rev-total-failed">$0.00</div>
                <div class="cube-label">Failed Payments</div>
            </div>
        </div>

        <!-- Revenue History User Details (shown when a user is selected) -->
        <div id="revenue-history-user-details" style="display:none;">
            <div class="revenue-history-user-header">
                <h3 id="revenue-history-user-name">User Name</h3>
                <button class="close-user-btn" onclick="Revenue.clearRevenueHistoryUserSelection()">✕</button>
            </div>
            ...
            <div id="revenue-history-records-container" style="margin-top: 20px;">
                ...
                <div id="revenue-history-records-list" style="max-height: 400px; overflow-y: auto;">
                    <div style="text-align:center;padding:20px;color:#888;">Loading revenue records...</div>
                </div>
            </div>
        </div>
    </div>
...
const Revenue = {
    ...
    init: function() {
        ...
        this.loadUsers();
        this.loadRevenueHistorySummary();
        this.bindEvents();
    },
    ...
    loadRevenueHistoryUsers: function() {
        ...
        .then(data => {
            if (data.success) {
                ...
                this.filteredRevenueHistoryUsers = [...this.allRevenueHistoryUsers];
                this.updateBadge('revenue-history-count', this.allRevenueHistoryUsers.length);
                
                // Process revenue history data from revenue_history column only
                this.processRevenueHistoryData(this.allRevenueHistoryUsers);
            }
        })
        .catch(error => {
    ...
    // PROCESS REVENUE HISTORY DATA - FROM revenue_history COLUMN ONLY
    // ============================================
    processRevenueHistoryData: function(users) {
        let totals = {
            totalInvestment: 0,
            totalPnl: 0,
            totalServerShare: 0,
        ...
        users.forEach(user => {
            // Get revenue history from the user's revenue_history column
            const history = user.revenue_history || [];
            const historyArray = Array.isArray(history) ? history : [];
            
            // Only process if there's actual history data
            if (historyArray.length > 0) {
                // Get the latest record for current balances
                const latestRecord = historyArray[0] || {};
                
                // Total investment comes from starting_balance of the latest record
                totals.totalInvestment += parseFloat(latestRecord.starting_balance || 0);
                
                // Total P&L from the latest record
                totals.totalPnl += parseFloat(latestRecord.profit || 0);
            }
            
            // Process all history records for revenue sums
            historyArray.forEach(record => {
                ...
                // User share - sum from all records
        ...
        this.revenueHistoryData = totals;
        this.renderRevenueHistorySummary(totals);
    },

    renderRevenueHistorySummary: function(totals) {
        this.updateCubeValue('rev-total-investment', totals.totalInvestment);
        this.updateCubeValue('rev-total-pnl', totals.totalPnl);
        this.updateCubeValue('rev-total-server-share', totals.totalServerShare);
        this.updateCubeValue('rev-total-user-share', totals.totalUserShare);
        this.updateCubeValue('rev-total-payments-made', totals.totalPaymentsMade);
        this.updateCubeValue('rev-total-payments-confirmed', totals.totalPaymentsConfirmed);
        this.updateCubeValue('rev-total-cancelled', totals.totalCancelled);
        this.updateCubeValue('rev-total-failed', totals.totalFailed);
    },
    ...
    updateCubeValue: function(id, value) {
        ...
        // Color coding: green for positive, red for negative, white for zero
        if (numValue > 0) {
    ...
    renderUserRevenueRecords: function(history) {
        ...
        if (!history || history.length === 0) {
            container.innerHTML = '<div style="text-align:center;padding:40px;color:#888;">No revenue records found for this user</div>';
            return;
        }
    ...
    renderRevenueHistoryModalUsers: function(users) {
        ...
        users.forEach(user => {
            const history = user.revenue_history || [];
            const historyArray = Array.isArray(history) ? history : [];
            const recordCount = historyArray.length;
    ...
    selectRevenueHistoryUserFromModal: function(userId, source) {
        ...
        // Update the search placeholder with selected user name
        document.getElementById('revenue-history-search-placeholder').textContent = 
            this.escapeHtml(user.fullname || 'N/A') + ' (ID: ' + user.id + ')';
        ...
        // Update summary cards with user data from revenue_history
    ...
    clearRevenueHistoryUserSelection: function() {
        ...
        document.getElementById('revenue-history-search-placeholder').textContent = 'Search users by name, email, or ID...';
        this.renderRevenueHistorySummary(this.revenueHistoryData);
    },

    loadRevenueHistorySummary: function() {
        setTimeout(() => {
            if (this.allRevenueHistoryUsers.length > 0) {
                this.processRevenueHistoryData(this.allRevenueHistoryUsers);
            }
        }, 500);
    },
    ...
    renderUserDetail: function(detailData, basicData) {
        ...
        // Parse daily target - handle various formats
        let dailyTargetData = {};
        if (dailyTarget) {
            if (typeof dailyTarget === 'string') {
                try {
                    ...
                    // If it's a string like "2026-08-10: $20.00", try to parse it
                    try {
        ...
        // Ensure dailyTargetData is an object
        if (typeof dailyTargetData !== 'object' || Array.isArray(dailyTargetData)) {
            dailyTargetData = {};
        }
        
        // Parse daily log - handle various formats
        let dailyLogData = {};
        if (dailyLog) {
        ...
        // Sort dates for daily target (YYYY-MM-DD format)
        ...
        // Sort dates for balance log (DD-MM-YYYY format)
        ...
        if (targetDates.length > 0) {
            ...
            targetDates.forEach(date => {
                ...
                // Check if this date has unusual activity in balance log
    ...
    // ============================================
    // UPDATE USER STATUS (Completed Tab)
    // ============================================
